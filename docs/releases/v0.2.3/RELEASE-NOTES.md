...
- 上线状态：已于 `2026-08-28T04:22:25Z` 发布到 V2 / slot-b
...
- 后端 102 项单元与 HTTP 回归、Python 编译、前端类型检查、生产构建和 Skill smoke 全部通过。
- 隔离候选与生产 V2 均以真实 Provider 生成 Markdown 与 HTML，并更新同名文件；两份文件的更新后哈希均发生变化，API 与磁盘字节一致。
- V2 重启后，更新版文件的大小与 SHA-256 均保持，当前对话 Token 统计也保持一致。
- 错写 `/tmp/**/outputs`、只落盘部分目标、新建空文件和把旧成果截空的模拟任务均进入可重试失败，不能出现成功终态。
- V1 / slot-a 未重启、未改配置、未改数据；发布前后本机与公网健康快照分别保持相同 SHA-256。
