...
本项目目前仍处于前期 Demo（开发者预览）阶段。在发布明确标记的稳定版本之前，应用 API、配置、持久化 Schema、Skill 协议、部署方式和目录结构均可能发生不向后兼容的变更，当前版本不承诺生产可用性或长期兼容性。

所有非兼容性升级必须在 `CHANGELOG.md` 中显著标记影响范围，并提供可执行的升级、备份和回滚说明；无法安全自动迁移时，必须明确给出人工迁移或重新初始化路径。使用方应固定版本，并在升级前备份数据和配置。
...
只有持久化数据契约变化时才升级 Schema；升级必须说明兼容范围、备份和回滚方法。能够安全迁移时应提供迁移器；无法安全自动迁移时，必须明确标记为非兼容性升级，并给出人工迁移或重新初始化路径。不得只为让展示数字一致而改写已有项目状态。
