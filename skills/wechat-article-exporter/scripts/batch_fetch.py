...
批量抓取微信文章
用法:
  python batch_fetch.py <urls_file> [formats_csv] [output_dir]
  python batch_fetch.py <urls_file> markdown,html,json outputs/wechat-articles/batch
...
from fetch_article import fetch_article, resolve_output_dir, save_article
...
def fetch_batch(urls: list, formats: list, output_dir: str = None) -> dict:
    ...
    """
    output_dir = resolve_output_dir(output_dir, "outputs/wechat-articles/batch")

    success = []   # [(title, [(format, filepath), ...]), ...]
    failed = []    # [{url, format, error}, ...]
        ...
        try:
            ...
            for fmt in formats:
                try:
                    ...
                    failed.append({"url": url, "format": fmt, "error": str(e)})
                    print(f"  [{fmt}] 失败: {e}", flush=True)
            if title and files:
                success.append((title, files))
        except Exception as e:
            failed.append({"url": url, "format": "*", "error": str(e)})
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python batch_fetch.py <urls_file> [formats_csv] [output_dir]")
        sys.exit(1)
    ...
    result = fetch_batch(urls, formats, output_dir)
    ...
    if result['failed']:
        ...
        for item in result['failed']:
            print(f"  - {item['url'][:60]} [{item['format']}]: {item['error']}")
