...
def _parse_html(value: str):
    """Load optional HTML dependencies only for network extraction/conversion."""
    try:
        from bs4 import BeautifulSoup
    except ImportError as exc:
        raise RuntimeError("BeautifulSoup4 is required for WeChat HTML extraction") from exc
...
def fetch_article(url: str, fmt: str = "markdown") -> dict:
    ...
    fetch_status = "success" if raw_text.strip() else "partial_no_body"
    fetch_note = (
        "正文已获取；作为研究证据时仍需核验文章引用的原始来源"
        if fetch_status == "success"
        else "页面可访问但未提取到正文；不得补写缺失内容"
    )
    ...
    if fmt == "html":
        ...
        content_obj = raw_text
    elif fmt == "json":
        content_obj = _build_json_export(
            title=title,
            author=author,
...
def _build_json_export(
    *,
    ...
    """Build the auditable JSON research record without network dependencies."""
    return json.dumps(
        {
            "标题": title,
            "作者": author,
            ...
            "是否原创": is_original,
            "抓取日期": fetched_at,
            "正文SHA256": hashlib.sha256(raw_text.encode("utf-8")).hexdigest(),
            "正文长度": len(raw_text),
            "抓取状态": fetch_status,
            "备注": fetch_note,
            "正文": raw_text,
...
def resolve_output_dir(output_dir: str = None,
                       default_relative: str = "outputs/wechat-articles") -> str:
    """Resolve and create an output directory contained by the current workspace."""
    workspace_root = Path.cwd().resolve()
    candidate = Path(output_dir).expanduser() if output_dir else Path(default_relative)
    ...
    try:
        ...
        raise ValueError("output_dir must stay inside the current conversation workspace") from exc
    target.mkdir(parents=True, exist_ok=True)
    resolved = target.resolve()
    try:
        resolved.relative_to(workspace_root)
    except ValueError as exc:
        raise ValueError("output_dir resolves outside the current conversation workspace") from exc
    return str(resolved)
